...
#  Code below was used by John campbell to make a set of nice looking graphs
###################################################

W3 <- ggplot() +
  geom_line(data = wb_wy,aes(x = wy, y= precip_mm_W3), colour="blue", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= strflow_mm_W3), colour = "red", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= et_mm_W3), colour = "dark gray", lwd=0.6) +
  xlab("Water Year (June 1)")+ ylab("Water (mm)")+ggtitle("W8")+
  theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ggtitle("W3")+
  scale_y_continuous(limits = c(floor(minval/250)*250, ceiling(maxval/250)*250), expand = c(0,0), breaks=seq(floor(minval/250)*250,ceiling(maxval/250)*250,250)) +
  scale_x_continuous(limits = c(1955, round(max(wb_wy$wy),-1)+5), expand = c(0,0), breaks=seq(1955,round(max(wb_wy$wy),-1)+5,10)) +
...
W6 <- ggplot() +
  geom_line(data = wb_wy,aes(x = wy, y= precip_mm_W6), colour="blue", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= strflow_mm_W6), colour = "red", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= et_mm_W6), colour = "dark gray", lwd=0.6) +
  xlab("Water Year (June 1)")+ ylab("Water (mm)")+ggtitle("W8")+
  theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  scale_y_continuous(limits = c(floor(minval/250)*250, ceiling(maxval/250)*250), expand = c(0,0), breaks=seq(floor(minval/250)*250,ceiling(maxval/250)*250,250)) +
  scale_x_continuous(limits = c(1955, round(max(wb_wy$wy),-1)+5), expand = c(0,0), breaks=seq(1955,round(max(wb_wy$wy),-1)+5,10)) +
...
W7 <- ggplot() +
  geom_line(data = wb_wy,aes(x = wy, y= precip_mm_W7), colour="blue", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= strflow_mm_W7), colour = "red", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= et_mm_W7), colour = "dark gray", lwd=0.6) +
  xlab("Water Year (June 1)")+ ylab("Water (mm)")+ggtitle("W8")+
  theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  scale_y_continuous(limits = c(floor(minval/250)*250, ceiling(maxval/250)*250), expand = c(0,0), breaks=seq(floor(minval/250)*250,ceiling(maxval/250)*250,250)) +
  scale_x_continuous(limits = c(1955, round(max(wb_wy$wy),-1)+5), expand = c(0,0), breaks=seq(1955,round(max(wb_wy$wy),-1)+5,10)) +
  theme(plot.title = element_text(size = 20, face = "bold", hjus=0.04, vjus=-9),
        ...
        axis.text.y=element_text(size=15, colour = "black"),
        axis.title.y=element_text(size=17, colour = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x=element_text(size=17, colour = "black", margin = margin(t = 10, r = 0, b = 0, l = 0)),
        legend.position = c(1, 0),
        legend.justification = c(1, 0),
        legend.margin = margin(0, 10, 7, 0),
        legend.key.size = unit(1, 'lines'),
...
W8 <- ggplot() +
  geom_line(data = wb_wy,aes(x = wy, y= precip_mm_W8), colour="blue", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= strflow_mm_W8), colour = "red", lwd=0.6) +
  geom_line(data = wb_wy,aes(x = wy, y= et_mm_W8), colour = "dark gray", lwd=0.6) +
  xlab("Water Year (June 1)")+ ylab("Water (mm)")+ggtitle("W8")+
  theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  scale_y_continuous(limits = c(floor(minval/250)*250, ceiling(maxval/250)*250), expand = c(0,0), breaks=seq(floor(minval/250)*250,ceiling(maxval/250)*250,250)) +
  scale_x_continuous(limits = c(1955, round(max(wb_wy$wy),-1)+5), expand = c(0,0), breaks=seq(1955,round(max(wb_wy$wy),-1)+5,10))+
...
cols <- c("Precipitation"="blue","Streamflow"="red","ET"="dark gray")
legend_only <- ggplot(data = wb_wy) +
  geom_line(aes(x = wy, y= precip_mm_W8, colour="Precipitation")) +
  geom_line(aes(x = wy, y= strflow_mm_W8, colour = "Streamflow")) +
  geom_line(aes(x = wy, y= et_mm_W8, colour = "ET")) +
  ...
  theme(
    ...
    legend.key.width = unit(1.2,"cm"),
    legend.background = element_rect(fill = "transparent", linetype = 1, size = 0.5, colour = "black"),
  )
legend_only
...
W7<-ggplotly(W7)
W8<-ggplotly(W8)
legend<-ggplotly(legend_only)
legend

plotfinal<-subplot(W3, W6, W7, W8, nrows=3, subplot_titles=',
                   shareX = T, shareY = T)

plotfinal
# this line writes the html file to create interactive graphs for the online book
htmlwidgets::saveWidget(as_widget(plotfinal), "Chapter2_fig7_seed_production_Cleavitt/seedfig_Cleavitt_ay.html")



legend <- get_legend(legend_only)
...
top_row <- plot_grid(p1, NULL, p2, NULL, NULL, NULL, p3, NULL, p4,  ncol = 3, nrow = 3, rel_widths = c(1,-0.19,1,1,-0.19,1,1,-0.19,1), rel_heights = c(1,-0.13,1,-0.13,-0.13,-0.13,1,-0.13,1), align="hv")

svg("water_budget.svg", height = 8.5, width = 11)
plot_grid(top_row, p5, nrow = 2, ncol = 1, rel_heights = c(1,0.15))
dev.off()
