...
# get the volume weighted
w.complete$po4pL<-w.complete$PO4_flux/w.complete$flow_mm


#######
pex<-aggregate(list(phosphate=w.complete$po4pL), by=list(Watershed = w.complete$Watershed, wyear=w.complete$wyear), FUN="mean", na.rm=T)

summary(pex$phosphate)

go<-ggplot(pex, aes(x=wyear, y=phosphate, col=Watershed, group=Watershed))+
